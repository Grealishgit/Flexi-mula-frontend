...
import logo from "../assets/toggle-logo.png";
...
const Navbar = ({ isCollapsed, toggleSidebar, isSidebarOpen, toggleSidebarVisibility }) => {
    ...
    return (
        <div
            className={`fixed top-0 left-0 ${isCollapsed ? "md:left-24" : "md:left-64"
                } right-0 flex justify-between items-center bg-white shadow-md px-4 md:px-10 p-4 transition-all duration-300 z-50`}
        >
            {/* Left Section */}
            <div className="flex flex-row items-center gap-3">
                <img onClick={() => navigate('/dashboard')} src={logo} alt="logo" className="sm:hidden" sizes={20} />
                <button
                ...
                >
                    {isSidebarOpen ? <RiCloseFill /> : <RiMenu2Fill />}
                </button>
            ...
            <div className="flex items-center gap-4">
                {/* Visible only on large screens (md and above) */}
                ...
                <div className="relative">
                    ...
                    {isProfileOpen && (
                        <div className="absolute right-0 mt-2 h-68 w-60 bg-white shadow-lg rounded-lg py-2">
                            ...
                            <MenuItem onClick={() => { navigate('/profile'); setIsProfileOpen(false); }} icon={<FiUser />} label="Profile" />
...
const ProfileAvatar = ({ onClick }) => (
    <div
        ...
        onClick={onClick}
    >
        <img src={profile} alt="User Avatar" className="rounded-full w-full aspect-square" />
    </div>
);
